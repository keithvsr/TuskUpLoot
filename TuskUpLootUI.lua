...
function UI.renderSelectedItem()
  if not UI.needsTitle or not UI.needsText or not UI.hasTitle or not UI.hasText or not UI.detailLinkFS then
    return
  end
  ...
  if not rollup or #rollup == 0 then
    ...
    for _, row in ipairs(rollup) do
      ...
      if row.hasAcquired then
        ...
        needs[#needs + 1] = string.format("%s", who)
        for _, gs in ipairs(row.gearSets or {}) do
          local phase = gs.phase ~= nil and string.format(" (Phase %s)", tostring(gs.phase)) or ""
          needs[#needs + 1] = string.format("    • %s%s", gs.name or gs.key, phase)
        end
      end
    end
    -- UI.hasTitle:ClearAllPoints()
    if #needs > 0 then
      -- print(item.name, "has needs")
      -- lines[#lines + 1] = "Needs Item:"
      UI.needsTitle:SetText("Needs Item:")
      -- lines[#lines + 1] = table.concat(needs, "\n")
      -- lines[#lines + 1] = ""
      needs[#needs + 1] = ""
      UI.needsText:SetText(table.concat(needs, "\n"))
      UI.hasTitle:ClearAllPoints()
      UI.hasTitle:SetPoint("TOPLEFT", UI.needsText, "BOTTOMLEFT", -textInset, -8)
    else
      UI.hasTitle:ClearAllPoints()
      local detailScrollChild = UI.detailScroll:GetScrollChild()
      UI.hasTitle:SetPoint("TOPLEFT", detailScrollChild, "TOPLEFT", 4, 0)
      -- print("No needs for", item.name)
    end

    if #has > 0 then
      -- print("someone has", item.name)
      -- lines[#lines + 1] = "Has Item:"
      UI.hasTitle:SetText("Has Item:")
      -- lines[#lines + 1] = table.concat(has, "\n")
...
function UI.ensureFrame()
  ...
  detailScroll:SetPoint("TOPLEFT", detailHeader, "BOTTOMLEFT", 0, -4)
  ...
  detailScrollChild:SetWidth(280)
  ...
  local needsText = detailScrollChild:CreateFontString("NeedsTextString", "OVERLAY", "GameFontHighlight")
  needsText:SetPoint("TOPLEFT", needsTitle, "BOTTOMLEFT", textInset, -4)
  needsText:SetJustifyH("LEFT")
  needsText:SetJustifyV("TOP")
  UI.needsText = needsText

  local hasTitle = detailScrollChild:CreateFontString("HasTitleString", "OVERLAY", "GameFontHighlightLarge")
  hasTitle:SetPoint("TOPLEFT", needsText, "BOTTOMLEFT", -textInset, -8)
